        ...
        <% if (cours && cours.length > 0) { %>
          <ul>
            <% cours.forEach(function(file) { %>
              <li>
                <a href="<%= file.link %>" target="_blank" class="inline-block px-3 py-1 bg-[#5C83BA] text-white rounded hover:bg-[#466a99] transition mb-1">
                  <%= file.name %>
                </a>
              </li>
            <% }); %>
        ...
        <div class="mb-3">
          ...
          <div class="file-label">
            <label class="custom-file-btn">
              Choisir un fichier
              <input type="file" name="pdfFile" accept=".pdf" required onchange="updateFileName(this)"/>
            </label>
